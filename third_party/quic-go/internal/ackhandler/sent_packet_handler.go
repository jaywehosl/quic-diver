...
func NewSentPacketHandler(
	...
	cc := newCongestionController(
		rttStats, connStats, initialMaxDatagramSize, brutalSendMbps, qlogger,
	)
...
// newCongestionController выбирает алгоритм: BRUTAL при заданной скорости, иначе обычный
// Cubic.
//
// Правка форка QUIC Diver. BRUTAL описан в docs/decisions/006-brutal.md: он не снижает
// скорость при потерях и потому отклоняется от RFC 9002 §7. Нулевая скорость означает, что
// ничего не меняется и работает стандартный алгоритм.
func newCongestionController(
	...
	if brutalSendMbps > 0 {
		return congestion.NewBrutalSender(
			...
			congestion.Bandwidth(brutalSendMbps)*1e6*congestion.BitsPerSecond,
		)
	}
