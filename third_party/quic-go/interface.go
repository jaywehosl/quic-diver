...
type Config struct {
	...
	// BrutalSendMbps sets a fixed sending rate, in megabits per second, and switches the
	// connection to the BRUTAL congestion controller (QUIC Diver fork).
	//
	// BRUTAL does not reduce its rate on loss: it treats packet loss as noise to be
	// compensated for rather than as a signal of congestion. This deviates from RFC 9002 §7,
	// deliberately and with the reasoning written down in docs/decisions/006-brutal.md. The
	// burden of not congesting the path falls on whoever sets the number: it must match the
	// real capacity of the link.
	//
	// Zero, the default, leaves the standard Cubic controller in place.
	BrutalSendMbps int
