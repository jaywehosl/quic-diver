...
//
// При работающем клиенте применяется сразу, без переподключения: скорость держится на одном
// числе внутри контроллера. Отвечает true, если применено на живой связи; false означает лишь,
// что применять некуда — число запомнено и уйдёт в дело при следующем запуске.
func SetBrutalUp(mbps int) bool {
	mu.Lock()
	prefs.brutalUp = mbps
	ctl := control
	mu.Unlock()
	...
	if mbps <= 0 {
		// Ноль и отрицательное означают «взять из сети». Выключить BRUTAL на живом
		// соединении нельзя — контроллер выбирается при рукопожатии.
		return false
	}
	return ctl.SetSendMbps(mbps)
}
