...
type Node struct {
	...
	// BrutalDownMbps и BrutalMeshMbps — потолки BRUTAL до прихода журнала.
	//
	// Настоящий источник этих чисел — журнал: там их правит владелец, и оттуда они расходятся
	// по сети. Но журнал приезжает через секунды после старта, а свежий узел получает его и
	// вовсе не сразу — вначале он просто стоит и ждёт. Всё это время он работал бы на обычном
	// Cubic, хотя человек задавал потолки при создании сети.
	//
	// Поэтому ключ развёртывания несёт их с собой, а узел кладёт сюда. Как только журнал
	// приходит, верх берут числа из него: файл не перечитывается, а журнал живой.
	BrutalDownMbps int `toml:"brutal_down_mbps"`
	BrutalMeshMbps int `toml:"brutal_mesh_mbps"`
}
